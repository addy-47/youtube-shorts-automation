    ...
    try:
        ...
        if isinstance(clip, CompositeVideoClip) and hasattr(clip, 'clips') and clip.clips:
            ...
            for subclipped in clip.clips:
                ...
                # Add to the list (original or modified)
                if subclipped_modified:
                    has_complex_subclippeds = True
                    modified_subclippeds.append(fixed_subclipped)
                else:
                    modified_subclippeds.append(subclipped)

            # If any subclippeds were modified, create a new composite
            if has_complex_subclippeds:
                modified_clip = CompositeVideoClip(modified_subclippeds, size=clip.size)
                modified_clip = modified_clip.with_duration(clip.duration)
                needs_cleaning = True
                logger.debug(f"Created modified composite clip with fixed subclippeds for clip {idx}")

        # Handle main clip callable attributes
        if has_callable_pos or has_callable_size:
            # Start with the modified clip if we already have one
            base_clip = modified_clip if modified_clip else clip
            ...
            if has_callable_size:
                try:
                    ...
                    # Apply the size as a resize
                    base_clip = base_clip.resized(width=mid_size[0], height=mid_size[1])
                    logger.debug(f"Fixed callable size in clip {idx}")
                except Exception as e:
                    logger.warning(f"Failed to fix callable size in clip: {e}")
                    # No fallback for size

            modified_clip = base_clip
            needs_cleaning = True

        # Use the modified clip if we created one, otherwise use the original
        render_clip = modified_clip if modified_clip else clip

        # Keep audio from original clip if available
        if hasattr(clip, 'audio') and clip.audio is not None:
            render_clip = render_clip.with_audio(clip.audio)

        # Now render the clip to a file
        try:
            ...
            logger.info(f"Successfully pre-rendered complex clip {idx} to {temp_path}")
            return temp_path
        except Exception as e:
            logger.error(f"Error pre-rendering clip {idx}: {e}")
            return None
    except Exception as e:
        logger.error(f"Error processing complex clip {idx}: {e}")
        return None
    finally:
        # Clean up modified clip if we created one
        if needs_cleaning and modified_clip is not None:
            try:
                modified_clip.close()
            except:
                pass
...
def render_clips_in_parallel(clips, output_file, fps=30, num_processes=None, logger=None, temp_dir=None, preset="veryfast", codec="libx264", audio_codec="aac", section_info=None):
    ...
    # Create a proper temporary directory if not provided
    if temp_dir is None:
        temp_dir = tempfile.mkdtemp(prefix="parallel_render_")
        created_temp_dir = True
        logger.info(f"Created temporary directory: {temp_dir}")
    else:
        # Ensure the provided temp directory exists
        if not os.path.exists(temp_dir):
            try:
                os.makedirs(temp_dir, exist_ok=True)
                logger.info(f"Created temporary directory: {temp_dir}")
                ...
                temp_dir = tempfile.mkdtemp(prefix="parallel_render_fallback_")
